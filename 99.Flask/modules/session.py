...
def create_users_session():
    basedir = os.path.join(appdir.users, session.get('username'))
...
def remove_session(anonymous_seconds=1800, username_seconds=-1):
    if session.get('username') == 'anonymous':
        ...
        remove_users_session(seconds=username_seconds)


def remove_users_session(seconds=-1):
    basedir = os.path.join(appdir.users, session.get('username'))
...
def destroy_session():
    destroy_users_session(seconds=-1)
...
def destroy_users_session(seconds=-1):
    basedir = appdir.users
