...
def constructor(settings):
    ...
    @router.route('/coffee/', methods=['GET'])
    def get_coffee():
    ...
    @router.route('/coffee/', methods=['POST'])
    def post_coffee():
        # Dynamically import modules by filename.
        model = import_model(filename, suffix='__coffee')
    ...
    @router.route('/population/', methods=['GET'])
    def get_population():
        # Set the parameters to pass when rendering the template.
        form = {}
        data = {}
    ...
    @router.route('/population/', methods=['POST'])
    def post_population():
        # Dynamically import modules by filename.
        model = import_model(filename, suffix='__population')
