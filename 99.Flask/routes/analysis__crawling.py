...
def constructor(settings):
    ...
    @router.route('/gasstaion/', methods=['GET'])
    def get_gasstaion():
    ...
    @router.route('/gasstaion/', methods=['POST'])
    def post_gasstaion():
        # Dynamically import modules by filename.
        model = import_model(filename, suffix='__gasstaion')
    ...
    @router.route('/sandwich/', methods=['GET'])
    def get_sandwich():
        # Set the parameters to pass when rendering the template.
        form = {}
        data = {}
    ...
    @router.route('/sandwich/', methods=['POST'])
    def post_sandwich():
        # Dynamically import modules by filename.
        model = import_model(filename, suffix='__sandwich')
